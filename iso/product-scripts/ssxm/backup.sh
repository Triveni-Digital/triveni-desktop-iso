...
if command -v tar >/dev/null 2>&1; then
    ...
    echo "WARNING: No tar implementation found in installer environment; cannot create SSXM backup archive."
fi
...
if [ "${#parts_to_scan[@]}" -eq 0 ]; then
    echo "WARNING: No partitions discovered for SSXM backup scan."
fi
...
for part in "${parts_to_scan[@]}"; do
    if mount "$part" /tmp/old_root 2>/dev/null; then
        ...
        # Layout A: mounted root filesystem contains /var/triveni/ssxm
        # Layout B: mounted /var/triveni partition contains /ssxm
        for candidate_root in \
            "/tmp/old_root/var/triveni/ssxm" \
            "/tmp/old_root/ssxm"; do
            if [ -d "$candidate_root" ]; then
                backup_root="$candidate_root"
        ...
        if [ "${#backup_items[@]}" -gt 0 ]; then
            echo "SSXM backup source found on $part at $backup_root. Attempting to create backup archive..."
